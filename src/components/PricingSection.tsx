    ...
    <section id="pricing" className="py-24 bg-gradient-surface relative">
      ...
      <div className="container mx-auto px-6 relative">
        <div className="text-center mb-16 animate-fade-in-up">
          ...
          <div className="inline-flex items-center bg-sl-slate-800 rounded-full p-1 border border-border-accent">
            <span className="px-4 py-2 text-text-muted">One-time Investment</span>
            <span className="px-4 py-2 bg-accent text-accent-foreground rounded-full font-medium">30-Day Delivery</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-2 max-w-6xl mx-auto">
          {plans.map((plan, index) => (
            <Card 
              key={index}
              className={`relative p-8 transition-all duration-500 hover:shadow-glow hover:-translate-y-2 animate-fade-in-up ${
                plan.popular 
                  ? 'bg-surface border-accent shadow-glow scale-105' 
            ...
            >
              ...
              <Button 
                className={`w-full py-3 font-semibold transition-all duration-300 ${
                  plan.popular
                    ? 'bg-accent text-accent-foreground hover:bg-accent-light shadow-glow'
                    : 'bg-secondary text-secondary-foreground hover:bg-surface-hover'
                }`}
              >
                {plan.name === "Enterprise Plus" ? "Get Custom Quote" : "Get Started"}
              </Button>
            </Card>
          ))}
        </div>
